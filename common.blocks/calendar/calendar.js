...
provide(BEMDOM.decl(this.name, /** @lends calendar.prototype */{
    ...
    _build : function() {
        ...
        this._selectedDayElem = this.findElem('day', 'state', 'current');
    },
    ...
    _onArrowClick : function(e) {
        var arrow = $(e.target);
        if(!this.hasMod(arrow, 'disabled')) {
            this.switchMonth(this.hasMod(arrow, 'direction', 'left')? -1 : 1);
        }
    ...
    _selectDayElem : function(element) {
        ...
        this.setMod(element, 'state', 'current');
        this._selectedDayElem = element;
    }
},  /** @lends calendar */ {
    live : function() {
        this.liveBindTo('arrow', 'pointerclick', this.prototype._onArrowClick);
        this.liveBindTo('day', 'pointerclick', this.prototype._onDayClick);
        return false;
    }
}));
